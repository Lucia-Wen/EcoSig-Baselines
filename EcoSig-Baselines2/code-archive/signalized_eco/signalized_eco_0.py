...
max_step = 1e3      #2e3    #1e3    #5e3    #3e3    | 1e3
max_dist= 5.6e3      #4e3    #1200   #2e3    #5400   | 500
...
class SigEcoEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # state = self.state
        if stop_mode:
            done = self.EndFlag or self.RedFlag # or self.RedFar #or bool(self.GreenStop>=5)
        else:
            done = self.EndFlag
        return state, reward, done, {"action_real": self.Agent_EV.action, 'rew_E': reward_E}
    ...
    def _check_green_stop(self):
        if self.state[4]>self.state[3] and self.state[0]<0.5:
            self.GreenStop += 1
        else:
            self.GreenStop = 0
    ...
    def _get_reward(self):
        ...
        if self.RedFlag:
            if stop_mode:
                self.reward_Red = -1000*(2-self.state_new[1]/max_dist) #(0.5+self.state[0]/max_vel)
            else:
                self.reward_Red = -250*(1.5-self.RedRatio)
        ...
        if mod =="dim3":
            reward = self.reward_Red  + self.reward_End + self.reward_red_far + self.reward_E #- reward_step
            ...
            if self.sequence_id[0]==signal_num-1:
                if self.Agent_EV.veh_state.dist_from_start >= self.traffic_signals.pos[i]:
                    ...
                    if self.state[4]<self.state[3]:
                        self.RedFlag=True
                        self.GreenFlag=False
                        self.RedRatio = (0.5*self.state[3]-abs(0.5*self.state[3]-self.state[4]))/self.state[3]
                    else:
...
def normalize_state(state):
    ...
    # self.state_new:
    if mod == "dim3":
        state[:,1] = state[:,1]/(max_dist/2)-1
        state[:,2] = state[:,2]/(max_step*dt/2)-1
